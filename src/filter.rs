...
impl<'a> SampleHttpFilter<'a> {
    ...
    pub fn new(
        ...
        let req_count = AtomicU8::new(config.count);
...
impl<'a> HttpFilter for SampleHttpFilter<'a> {
    ...
    fn on_request_headers(
        ...
        match filter_ops.request_header(":path")? {
            ...
            Some(path) if path == "/secret" => {
                ...
                let mut count = self.config.count;
                loop {
                    let req = self.http_client.send_request(
                        &self.config.cluster,
                        &[
                            (":method", "GET"),
                            (":path", "/authz"),
    ...
    fn on_http_call_response(
        ...
        let prev = self.active_request_count.fetch_sub(1, Ordering::SeqCst);
